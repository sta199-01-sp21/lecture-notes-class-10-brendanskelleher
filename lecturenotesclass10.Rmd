...
My Answer: Very close. The true probabilities are 37.5% (blue), 50% (red),
and 12.5% (white).
...
My Answer: the probability will be somewhere around 40%?
...
My Guess: Square match will be more prevalent
...
two_roll_results <- two_roll_results %>% 
  mutate(square_match = (roll_1 * roll_1) == (roll_2 * roll_2),
         abs_match = 4 == abs(roll_2 - roll_1))
two_roll_results
```

```{r summarize_rolls}
two_roll_results %>% 
  summarize(prop_square = sum(square_match) / n(),
            prop_abs = sum(abs_match) / n())
```

Conclusion: Square match was more prevalent by about 5%!
